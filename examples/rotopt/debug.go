...
func txtToFloatDict(txtPath string) map[int]float64 {
	// Each line in the text file should be in the format "level 	time(sec)"
	...
	floatDict := make(map[int]float64)
	scanner := bufio.NewScanner(file)
	...
	for scanner.Scan() {
		...
		if err != nil {
			fmt.Println("Invalid level:", parts[0])
			continue
		}
		...
		if err != nil {
			fmt.Println("Invalid time:", parts[1])
			continue
		}

		floatDict[level] = time
	}
